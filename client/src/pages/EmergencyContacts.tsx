...
export default function EmergencyContacts() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/contacts?user=${user?.id || '1'}`);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const url = editId ? `http://localhost:5000/api/contacts/${editId}` : 'http://localhost:5000/api/contacts';
